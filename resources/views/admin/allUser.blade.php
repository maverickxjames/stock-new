                                            ...
                                            @foreach ($users as $user)
                                                <tr>
                                                    ...
                                                    <td class="wallet-container">
                                                        ...
                                                        <button class="add-fund-button"
                                                            onclick="addFund({{ $user->id }})">Add Fund</button>
                                                    </td>
                                                    <td>
                                                        @php
                                                        ...
                                                        @if ($referralCount > 0)
                                                            <button class="btn btn-primary" data-toggle="modal"
                                                                ...
                                                                onclick="loadReferralTeam('{{ $user->user_id }}')">
                                                                View
                                                            </button>
                                                        @endif
  ...
  <div class="modal-dialog modal-xl" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Referral Team</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span>&times;</span>
        </button>
      </div>
      <div class="modal-body" id="referralTeamContent">
        <p>Loading...</p>
      </div>
    </div>
  </div>
    ...
    <script>
function loadReferralTeam(userId) {
    fetch(`/admin/referrals/${userId}`)
        .then(response => response.text())
        .then(data => {
            document.getElementById('referralTeamContent').innerHTML = data;
        })
        .catch(error => {
            console.error("Error fetching referral data:", error);
            document.getElementById('referralTeamContent').innerHTML = '<p>Error loading data.</p>';
        });
}
</script>
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if (response.success) {
                                Swal.fire('Success', `Successfully added ₹${amount} to the wallet.`,
                                    'success').then(() => {
                                    location.reload();
                                });
